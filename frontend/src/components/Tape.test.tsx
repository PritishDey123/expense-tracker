...
describe('Tape', () => {
  it('renders rows most-recent-first as given by items order', () => {
    render(<Tape items={[A]} loading={false} error={null} hasNext={false} onLoadMore={vi.fn()} onEditSave={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByText('Food')).toBeInTheDocument()
  })

  it('shows the empty state when there are no items and no error', () => {
    render(<Tape items={[]} loading={false} error={null} hasNext={false} onLoadMore={vi.fn()} onEditSave={vi.fn()} onDelete={vi.fn()} />)
    expect(screen.getByText(/no expenses yet/i)).toBeInTheDocument()
  })

  it('shows an error banner and no rows when loading failed', () => {
    render(<Tape items={[]} loading={false} error="Could not retrieve expenses" hasNext={false} onLoadMore={vi.fn()} onEditSave={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('calls onLoadMore when the sentinel intersects and more pages exist', () => {
    ...
    render(<Tape items={[A]} loading={false} error={null} hasNext={true} onLoadMore={onLoadMore} onEditSave={vi.fn()} onDelete={vi.fn()} />)
